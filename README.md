...
默认扫描根目录是应用内已忽略的 `workspace/`，不会枚举仓库或安装目录的同级文件夹。可把待扫描目录放入其中，或使用页面导入；确需读取其他受控根目录时，显式传入 `-workspace-root <PATH>`。

服务只接受回环监听地址，也不提供公网登录层。需要在 Linux 服务器上运行时，仍绑定 `127.0.0.1` 并通过 SSH 隧道访问；不要改成 `0.0.0.0`。完整升级、备份、恢复、健康检查和卸载步骤见 [运维指南](./docs/OPERATIONS.md)。
